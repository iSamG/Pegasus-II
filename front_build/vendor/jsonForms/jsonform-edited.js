...
jsonform.fieldTemplate = function(inner) {
  return '<div class="form-group jsonform-error-<%= keydash %>' +
    ...
    '<% if (node.title && !elt.notitle) { %>' +
      '<label class=" " for="<%= node.id %>" style="font-size : 16px; color: black"><%= node.title %></label>' +/*class = control-label*/
    '<% } %>' +
    '<div class=" col-md-12">' +/*class = controls*/
...
jsonform.elementTypes = {
  ...
  'root': {
    'template': '<div class="col-md-6 col-sm-8 col-xs-10 center-block"><%= children %></div>'
  },
  ...
  'checkbox':{
    'template': '<label class="checkbox"><input class="" type="checkbox" id="<%= id %>" ' +
  ...
  'radios':{
    'template': '<div id="<%= node.id %>" style="margin-left: 20px;"><% _.each(node.options, function(key, val) { %><label class="radio"><input class="" type="radio" <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> checked="checked" <% } %> name="<%= node.name %>" value="<%= (key instanceof Object ? key.value : key) %>"' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
  ...
  'checkboxes':{
    'template': '<div style="margin-left: 20px;"><%= choiceshtml %></div>',
    'fieldtemplate': true,
    'inputfield': true,
    'onBeforeRender': function (data, node) {
      ...
      var template = '<label class="checkbox">' +
        '<input type="checkbox" class="" <% if (value) { %> checked="checked" <% } %> name="<%= name %>" value="1"' +
        '<%= (node.disabled? " disabled" : "")%>' +
  ...
  'submit':{
    'template':'<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="btn  <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>'
  },
  'button':{
